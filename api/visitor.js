export default async function handler(req, res) {
  ...
  async function getData() {
    ...
    return data.result ? JSON.parse(data.result) : { pengunjung: [] };
  }
  ...
  try {
    // ==== GET → tampilkan semua pengunjung ====
    if (req.method === "GET") {
    ...
    // ==== POST → catat tamu pertama kali ====
    if (req.method === "POST") {
      if (!tamu)
        return res.status(400).json({ error: "Parameter ?to (nama tamu) wajib diisi" });
